...
impl Zeroize for Source {
    fn zeroize(&mut self) {
        match self {
            ...
            Self::Command(command) => command.zeroize(),
...
impl Zeroize for CommandCredential {
    fn zeroize(&mut self) {
...
impl Drop for CommandCredential {
    fn drop(&mut self) {
        self.zeroize();
    }
}
impl ZeroizeOnDrop for CommandCredential {}
...
impl Auth {
    ...
    pub fn api_key_command(
        program: PathBuf,
        refresh_interval: Duration,
        timeout: Duration,
    ) -> Result<Self, AuthError> {
        if program.as_os_str().is_empty() || timeout.is_zero() {
            return Err(AuthError::InvalidCredentials);
        }
        Ok(Self {
            mode: AuthMode::ApiKey,
            source: Mutex::new(Source::Command(Box::new(CommandCredential {
                program,
                credential: None,
                generation: 0,
                loaded_at: None,
    ...
    pub(crate) async fn headers(&self) -> Result<(HeaderMap, u64), AuthError> {
        ...
        match &mut *source {
            ...
            Source::Command(command) => {
                command.refresh(false).await?;
                let credential = command
                    .credential
                    .as_ref()
                    .ok_or(AuthError::InvalidCredentials)?;
                Ok((
                    headers(credential.expose_secret(), None)?,
                    command.generation,
                ))
            }
    ...
    pub(crate) async fn recover_unauthorized(
        ...
        match &mut *source {
            Source::ApiKey(_) => {
                // A static API key cannot be refreshed locally, but an OpenAI-compatible
                // gateway may have repaired its own upstream authentication. The caller
                // may safely retry this pre-generation rejection.
                Ok(())
            }
            Source::Command(command) => {
                if command.generation == rejected_generation {
                    command.refresh(true).await?;
                }
                Ok(())
            }
            Source::ChatGpt(managed) => {
...
impl CommandCredential {
    async fn refresh(&mut self, force: bool) -> Result<(), AuthError> {
        if !force
            && self.credential.is_some()
            && self
                .loaded_at
                .is_some_and(|loaded| loaded.elapsed() < self.refresh_interval)
        {
            return Ok(());
        }
        ...
        process
            ...
            .kill_on_drop(true);
        let output = timeout(self.timeout, process.output())
            .await
        ...
        self.credential = Some(SecretString::new(value.to_owned()));
