...
pub enum SessionCommand {
    ...
    ProviderUsage {
        ...
        #[serde(default, skip_serializing_if = "Option::is_none")]
        representation: Option<crate::context_cost::RepresentationObservation>,
    },
    ProviderCost {
        request: Uuid,
        call: Uuid,
        cost_usd: Option<crate::inference::UsdCost>,
    },
    Feedback {
        message: String,
    },
    ...
    ContextProjected {
        source_revision: u64,
        #[serde(default, skip_serializing_if = "Option::is_none")]
        view: Option<crate::context::ContextView>,
...
impl SessionState {
    ...
    pub(crate) fn create(id: SessionId, creation: SessionCreation) -> Self {
        ...
        let settled_history_items = history.len();
        let title = imported
            .as_ref()
            .map(|source| {
                source
    ...
    pub(crate) fn apply(
        ...
        if self.active_request.is_none() {
            ...
            self.settled_history_items = self.history.len();
        }
        Ok(())
    }
...
pub struct ProviderCostSummary {
    pub total: crate::inference::UsdCost,
    pub uncertain: bool,
}
