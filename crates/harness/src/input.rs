...
/// Serialize controller-owned context pages without applying user attachment
/// count or byte limits. Artifacts are always integrity-checked before use.
pub fn materialize_internal_context_media(
    media: &[crate::inference::InternalContextMedia],
    artifacts: &ArtifactStore,
) -> Result<Value, StoreError> {
    Ok(json!({
        "role":"user",
        "content":materialize_internal_context_parts(media, artifacts)?,
    }))
}

pub fn materialize_internal_context_parts(
    media: &[crate::inference::InternalContextMedia],
    artifacts: &ArtifactStore,
) -> Result<Vec<Value>, StoreError> {
    if media.is_empty() {
        return Err(StoreError::Invalid("internal context media is empty"));
    }
    let mut parts = Vec::with_capacity(media.len() * 2);
    for page in media {
        if page.locator.is_empty()
            || page.locator.len() > 512
            || page.locator.chars().any(char::is_control)
        {
            return Err(StoreError::Invalid("invalid internal context page locator"));
        }
        let bytes = artifacts.read(page.digest)?;
        let valid = match page.mime.as_str() {
            "image/png" => bytes.starts_with(b"\x89PNG\r\n\x1a\n"),
            "image/jpeg" => bytes.starts_with(b"\xff\xd8\xff"),
            "image/gif" => bytes.starts_with(b"GIF87a") || bytes.starts_with(b"GIF89a"),
            "image/webp" => bytes.starts_with(b"RIFF") && bytes.get(8..12) == Some(b"WEBP"),
        ...
        if !valid {
            return Err(StoreError::Integrity(
                "internal context media has an invalid type or signature",
            ));
        }
        parts.push(json!({
            "type":"input_text",
            "text":format!("[Host context page: {}]", page.locator),
        }));
        let image = if let Some(file_id) = &page.provider_file_id {
            if file_id.is_empty()
                || file_id.len() > 512
                || !file_id.bytes().all(|byte| byte.is_ascii_graphic())
            {
                return Err(StoreError::Invalid("invalid provider file identity"));
            }
            json!({"type":"input_image","file_id":file_id,"detail":"high"})
        } else {
            json!({
                "type":"input_image",
                "image_url":format!("data:{};base64,{}", page.mime, STANDARD.encode(bytes)),
                "detail":"high",
            })
        };
        parts.push(image);
