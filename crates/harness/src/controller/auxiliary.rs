...
impl Host {
    ...
    async fn classify_ordinary_call(
        ...
        let (artifacts, model, input) = {
            ...
            self.validate_session_admission(&session_state)?;
            (
        ...
        self.store
            .lock()
            .await
            .begin_classification(session, request)?;
        ...
        loop {
            ...
            let response = self
                ...
                .await;
            super::record_provider_cost(&self.store, session, request, call, &response).await?;
    ...
    pub(super) async fn run_auxiliary(
        ...
        loop {
            ...
            let response = self
                ...
                .await;
            super::record_provider_cost(&self.store, session.id, request, call, &response).await?;
            let retryable_rejection = response.retryable_pre_generation_rejection();
            let tokens = response.accounted_tokens();
            let successful = response.failure.is_none()
                && response
...
fn ordinary_classification_request(
    ...
    .and_then(|request| {
        request.with_json_schema(
            "ordinary_classification",
            json!({
                "type": "object",
                "properties": {"kind": {"type": "string", "enum": ["information", "action"]}},
                "required": ["kind"],
...
mod tests {
    ...
    #[test]
    fn ordinary_classifier_requires_a_closed_json_output_schema() {
        let request = ordinary_classification_request(
            crate::inference::ModelSettings::default(),
            SessionId::new(),
            vec![json!({"role":"user","content":[{"type":"input_text","text":"Understand the codebase"}]})],
        )
        .unwrap();
        for transport in [
            crate::inference::Transport::Http,
            crate::inference::Transport::WebSocket,
        ] {
            ...
            assert!(
                wire["instructions"]
                    .as_str()
                    .unwrap()
                    .contains(r#"{"kind":"action"}"#)
            );
        }
        assert!(
            parse_ordinary_kind(r#"{"kind":"action","action":"Understand the codebase"}"#).is_err()
        );
    }

    #[test]
    fn classifier_schema_is_part_of_cache_identity_and_survives_rerouting() {
        let session = SessionId::new();
        let input =
            vec![json!({"role":"user","content":[{"type":"input_text","text":"Explain this"}]})];
        let classifier = ordinary_classification_request(
            crate::inference::ModelSettings::default(),
            session,
            input.clone(),
        )
        .unwrap();
        let plain = InferenceRequest::new(
            crate::inference::ModelSettings::default(),
            input,
            Vec::new(),
            CLASSIFICATION_INSTRUCTIONS.into(),
        ...
        .unwrap();
        assert!(
            plain.wire(crate::inference::Transport::Http)["text"]
                .get("format")
                .is_none()
        );
        assert_ne!(classifier.cache_lineage(), plain.cache_lineage());
        let rerouted = classifier
            .clone()
            .with_prompt_cache_key("routing-check")
            .unwrap();
        assert_eq!(
            rerouted.wire(crate::inference::Transport::Http)["text"]["format"],
            classifier.wire(crate::inference::Transport::Http)["text"]["format"],
        );
        let restored = rerouted.with_prompt_cache_key(session.to_string()).unwrap();
        assert_eq!(restored.cache_lineage(), classifier.cache_lineage());
        let different_schema = classifier
            .clone()
            .with_json_schema(
                "ordinary_classification",
                json!({
            ...
            .unwrap();
        assert_ne!(different_schema.cache_lineage(), classifier.cache_lineage());
    }
