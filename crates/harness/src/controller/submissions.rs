                    ...
                    WorkIntent::Ordinary { limits, policy, .. } => {
                        ...
                        match kind {
                            OrdinaryKind::Information => {
                                // Use the ordinary auxiliary lifecycle so Escape
                                // reaches the answer phase after classification,
                                // and publication still settles every outcome.
                                return self
                                    .execute_auxiliary(
                                        session,
                                        submission.id,
                                        crate::auxiliary::ordinary_conversation_spec(),
                                    )
                                    .await;
                            }
