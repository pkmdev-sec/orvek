...
pub enum RemoteClientError {
    ...
    InvalidEndpoint,
    /// A non-loopback endpoint attempted to use plaintext HTTP.
    #[error("remote memory endpoint must use HTTPS outside loopback")]
    InsecureTransport,
...
impl RemoteMemoryClient {
    ...
    pub fn new(
        ...
        if endpoint.scheme() == "http" && !url_host_is_loopback(&endpoint) {
            return Err(RemoteClientError::InsecureTransport);
        }
...
mod tests {
    ...
    #[test]
    fn client_requires_encryption_outside_loopback() {
        for endpoint in [
            "http://localhost:8080/",
            "http://127.0.0.1:8080/",
            "http://[::1]:8080/",
            ...
            assert!(RemoteMemoryClient::new(endpoint, "alice".to_owned(), token).is_ok());
        }

        let token = RemoteToken::new("test-token".to_owned()).unwrap();
        assert!(matches!(
            RemoteMemoryClient::new("http://memory.example/", "alice".to_owned(), token),
            Err(RemoteClientError::InsecureTransport)
        ));
    }
