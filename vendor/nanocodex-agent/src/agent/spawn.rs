...
pub(super) fn build_agent<S>(
    ...
    context_backend: Option<Arc<dyn crate::session::compaction::ContextBackend>>,
    context_upgrade: Option<ContextContract>,
    ...
    S::Future: AgentSend,
{
    ...
    if context_upgrade.is_some()
        && (context_backend.is_none()
            || resume
                .as_ref()
                .is_none_or(|snapshot| snapshot.version() != 1))
    {
        return Err(NanocodexError::InvalidSessionSnapshot(
            "context contract upgrades require a legacy snapshot and an archive backend".to_owned(),
        ));
    }
    if let Some(snapshot) = &resume {
        match (&context_backend, snapshot.context_checkpoint()) {
            (Some(backend), Some(_)) => backend.validate(snapshot)?,
            (None, Some(_)) => {
                return Err(crate::session::compaction::ContextError::InvalidArchive {
                    reason: "this snapshot requires its context backend",
                }
                .into());
            }
            _ => {}
