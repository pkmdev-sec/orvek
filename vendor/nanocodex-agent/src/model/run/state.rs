...
use crate::session::{
    SessionId,
    compaction::{ContextBackend, ContextCheckpoint, ContextRecord},
};
...
pub(super) struct ConversationState {
    ...
    pub(super) archive: Option<ContextCheckpoint>,
    archive_backend: Option<Arc<dyn ContextBackend>>,
}
...
impl ConversationState {
    ...
    pub(super) fn begin_response_group(&mut self) {
        if let Some(checkpoint) = &mut self.archive {
            checkpoint.model_generation = checkpoint.model_generation.saturating_add(1);
        }
    }
    ...
    pub(super) fn append(&mut self, items: impl IntoIterator<Item = ResponseItem>) -> Result<()> {
        self.append_with_outcome(items, None)
    }

    pub(super) fn append_with_outcome(
        &mut self,
        items: impl IntoIterator<Item = ResponseItem>,
        tool_success: Option<bool>,
    ) -> Result<()> {
        let (Some(checkpoint), Some(backend)) = (&mut self.archive, &self.archive_backend) else {
            self.managed.append(items);
            return Ok(());
        };
        for mut original in items {
            assign_missing_response_item_id(&mut original);
            let mut candidate = self.managed.clone();
            let Some(visible) = candidate.append_recorded(original.clone()) else {
                continue;
            };
            let mut next = checkpoint.clone();
            next.sequence = next.sequence.checked_add(1).ok_or(
                crate::session::compaction::ContextError::InvalidArchive {
                    reason: "archive sequence exhausted",
                },
            )?;
            backend.record(ContextRecord {
                checkpoint: &next,
                original: &original,
                visible: &visible,
                tool_success,
    ...
    pub(super) fn open_archive(
        ...
        let imported = self.archive.is_none();
        let mut checkpoint = backend.open(session_id, self.archive.as_ref())?;
        if imported {
            for item in self.managed.history() {
    ...
    pub(super) fn append_canonical_context(
        ...
        self.append([canonical_developer_context, canonical_context.clone()])?;
