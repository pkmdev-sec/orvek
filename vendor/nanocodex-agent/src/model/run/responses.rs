    ...
    S::Future: AgentSend,
{
    pub(super) async fn perform_model_call(
        ...
        if self.context_backend.is_some() {
            if self.local_context_enabled() {
                self.compact_local(call_index.saturating_sub(1), conversation, factory, false)
                    .await?;
            } else {
                let snapshot = ContextSnapshot::reconstruct(std::slice::from_ref(
                    conversation.canonical_context.as_ref(),
                ));
                self.maybe_compact(
                    call_index.saturating_sub(1),
                    conversation,
                    factory,
                    CompactionContext {
