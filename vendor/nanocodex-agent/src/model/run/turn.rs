    ...
    S::Future: AgentSend,
{
    pub(crate) async fn compact(
        ...
        if self.local_context_enabled() {
            let outcome = {
                let compaction = self.compact_local(
            ...
            return match outcome {
                ...
                Some(Err(error)) => Ok(ModelCompactOutcome::Failed { error, checkpoint }),
                None => {
                    self.compaction_cancelled()?;
    ...
    pub(crate) fn emit_cancelled_before_start(
        ...
        self.events.emit(
            ...
            RunStarted {
                input_budget_tokens: self
                    .context_backend
                    .as_ref()
                    .map(|backend| backend.policy().input_tokens),
                mode: "openai_model",
    ...
    pub(super) async fn prepare_follow_on_turn(
        ...
        let compacted = if self.context_backend.is_some() {
            // Local preflight runs after the accepted prompt and current context
            // are recorded, so an over-budget request cannot lose new instructions.
            Some(false)
        } else {
        ...
        if compacted && !self.local_context_enabled() {
            session.context.require_full_reinjection();
        }
    ...
    pub(super) fn commit_interrupted_checkpoint(&mut self) -> Result<ModelCheckpoint> {
        ...
        session
            .conversation
            .append_with_outcome(aborted_outputs, Some(false))?;
    ...
    pub(super) async fn drive_session(
        ...
        loop {
            ...
            if !response.output_items.is_empty() {
                session.conversation.begin_response_group();
            }
