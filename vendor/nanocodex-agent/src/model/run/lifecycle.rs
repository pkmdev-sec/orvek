    ...
    S::Future: AgentSend,
{
    pub(super) async fn maybe_compact(
        ...
        if self.local_context_enabled() {
            return self
                .compact_local(after_model_call_index, conversation, factory, false)
                .await;
        }
        let CompactionContext { snapshot, phase } = context;
        let Some(auto_compact_token_limit) = self
            .context_backend
            .as_ref()
            .map(|backend| backend.policy().input_tokens * 9 / 10)
            .or_else(|| compaction::auto_compact_token_limit(self.model.as_str()))
        else {
            return Ok(false);
        };
    ...
    pub(super) fn compaction_cancelled(&mut self) -> Result<()> {
