...
use crate::{AgentEventKind, CostStatus, EstimatedUsdCost, MessagePhase, ToolOutputBody, Usage};
...
pub struct CompactionStarted {
    /// Whether the caller explicitly requested this operation.
    #[serde(default)]
    pub manual: bool,
...
pub struct CompactionCompleted {
    ...
    /// Time from attempt start to the first provider event.
    #[serde(default)]
    pub time_to_first_event_ns: Option<u64>,
    /// Time from attempt start to first compaction output.
    pub time_to_first_output_ns: Option<u64>,
    /// Provider token usage when supplied.
    pub usage: Option<Usage>,
    /// Compaction carrier, absent for historical provider events.
    #[serde(default)]
    pub strategy: Option<String>,
    /// Complete request estimate before local compaction.
    #[serde(default)]
    pub before_tokens: Option<u64>,
    /// Complete request estimate after local compaction.
    #[serde(default)]
    pub after_tokens: Option<u64>,
    /// Total bitmap pages retained by the resulting request.
    #[serde(default)]
