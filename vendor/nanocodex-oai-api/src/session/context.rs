...
impl ContextManager {
    pub(super) fn from_normalized(items: Vec<ResponseItem>) -> Self {
        let calls = CallIds::from_items(items.iter());
        Self {
            items: ResponseHistory::new(items),
...
fn truncate_output_content(items: &mut Vec<FunctionOutputContent>, token_limit: usize) {
    ...
    for mut item in std::mem::take(items) {
        match &mut item {
            ...
            | FunctionOutputContent::InputAudio { .. } => output.push(item),
        }
    }
