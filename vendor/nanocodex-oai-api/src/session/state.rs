...
impl ManagedSessionState {
    ...
    pub fn resume(mut items: Vec<ResponseItem>) -> Result<Self, ManagedSessionStateError> {
        ...
        if items.iter().any(|item| {
            matches!(
                item,
                ResponseItem::Other(_) | ResponseItem::CompactionTrigger {}
            )
        }) {
            return Err(ManagedSessionStateError::UnsupportedHistoryItem);
        }
        // Restored history is already the authoritative model-visible form.
        // Reapplying output truncation would remove more text on every resume.
        let mut state = Self::new(Vec::new());
        state.context = ContextManager::from_normalized(items);
    ...
    /// Appends one item and returns the normalized item actually retained. This
    /// allows an archive to keep the original and model-visible forms together.
    #[doc(hidden)]
    pub fn append_recorded(&mut self, item: ResponseItem) -> Option<ResponseItem> {
        let previous_len = self.context.len();
        self.context.record_items([item]);
        self.projection_revision = self.projection_revision.saturating_add(1);
        (self.context.len() > previous_len)
            .then(|| self.context.iter().last().cloned())
            .flatten()
    }
    ...
    /// Installs a structurally validated application projection and invalidates
    /// provider continuation. The caller supplies its profile-aware token estimate.
    #[doc(hidden)]
    pub fn install_projection(
        ...
        if self.projection_revision != expected_revision {
            return Err(ManagedSessionStateError::StaleProjection);
        }
        if items.is_empty() {
            return Err(ManagedSessionStateError::EmptyHistory);
        }
        ...
        self.context.update_token_info(Some(&Usage {
    ...
    /// Generation of all history mutations, including appends. This is separate
    /// from the replacement revision used by append-only rollout persistence.
    #[doc(hidden)]
    #[must_use]
...
pub enum ManagedSessionStateError {
    /// The prepared replacement refers to an older history generation.
    #[error("context projection has a stale history revision")]
    StaleProjection,
