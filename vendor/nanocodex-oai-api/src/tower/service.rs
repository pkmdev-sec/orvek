use crate::{
    AgentEventKind, ModelConfig, OpenAiAuthMode, OpenAiAuthSnapshot, ResponsesHistory,
    ResponsesTransport,
...
impl ResponsesService {
    ...
    fn encode_request(
        ...
        if self
            ...
            .is_some_and(|limit| encoded.raw().get().len() > limit)
        {
            return Err(ResponsesServiceError::invalid_attempt_state(
                "serialized context request exceeds its configured byte limit",
                FailurePhase::Encode,
...
mod tests {
    ...
    #[test]
    fn request_byte_limit_includes_wire_metadata_before_sending() {
        let mut config = crate::ModelConfig::default();
        config.context_request_limit = Some(128);
        let service = super::ResponsesService::new(Arc::new(config));
        let (events, mut receiver) = crate::EventSink::channel("bounded-wire-fixture".to_owned());
        let factory = crate::ResponsesAttemptFactory::new(
            crate::responses::RequestProfile::new(
                "bounded-wire-fixture",
                "bounded-wire-fixture",
                Arc::from([]),
            ),
            events,
        ...
        let items = vec![crate::responses::ResponseItem::message(
            crate::responses::MessageRole::User,
            [crate::responses::ContentItem::input_text("fixture")],
        )];
        assert!(serde_json::to_vec(&items).unwrap().len() < 128);
        let history = crate::responses::ResponseHistory::new(items);
        let request = factory.generation(
            ...
            None,
            crate::Model::Sol,
            crate::Thinking::Medium,
            false,
        );
        assert_eq!(request.input_items().count(), 1);
        for transport in [
            ...
            assert!(
                service
                    .encode_request(&ConnectionState::new(), &request, transport)
                    .is_err()
            );
        }
