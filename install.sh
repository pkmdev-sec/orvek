...
repository_url="https://github.com/pkmdev-sec/orvek"
install_dir=${ORVEK_INSTALL_DIR:-}
if [ -z "$install_dir" ]; then
    if [ -z "${HOME:-}" ]; then
        echo "error: HOME is not set; set ORVEK_INSTALL_DIR to an absolute directory" >&2
        exit 1
    fi
...
case "$install_dir" in
    /*) ;;
    *)
        echo "error: ORVEK_INSTALL_DIR must be an absolute path" >&2
        exit 1
        ;;
esac
...
temporary_dir=$(mktemp -d "${TMPDIR:-/tmp}/orvek-install.XXXXXX")
staged_binary=
cleanup() {
...
case "$version" in
    v[0-9]* ) ;;
    *)
        echo "error: could not determine the latest orvek release from $latest_url" >&2
        exit 1
        ;;
esac
case "$version" in
    */* | *\?* | *\#*)
        echo "error: GitHub returned an invalid orvek release version: $version" >&2
        exit 1
        ;;
esac

archive_name="orvek-$target-$version.tar.gz"
...
(
    ...
    if command -v sha256sum >/dev/null 2>&1; then
        ...
        echo "error: install sha256sum or shasum to verify the orvek archive" >&2
        exit 1
    fi
)

package="orvek-$target-$version"
expected_binary="$package/orvek"
...
if [ ! -f "$binary" ] || [ -L "$binary" ]; then
    echo "error: release archive's orvek entry is not a regular file" >&2
    exit 1
fi

mkdir -p "$install_dir"
destination="$install_dir/orvek"
if [ -d "$destination" ]; then
    echo "error: installation destination is a directory: $destination" >&2
    exit 1
fi
staged_binary=$(mktemp "$install_dir/.orvek.XXXXXX")
install -m 755 "$binary" "$staged_binary"
if ! "$staged_binary" --version >/dev/null 2>&1; then
    echo "error: downloaded orvek binary cannot run on this system" >&2
    exit 1
fi
mv -f "$staged_binary" "$destination"
staged_binary=

echo "Installed orvek $version to $destination"
case ":${PATH:-}:" in
    *":$install_dir:"*) ;;
    *) echo "Add $install_dir to PATH to run orvek." ;;
esac
