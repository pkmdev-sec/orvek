...
- `just test`: Run tests
- `python3 scripts/generate-codebase-graph.py --check`: Verify the codebase navigation graph is current.

## Codebase orientation

- Begin a fresh-session investigation with [`CODEBASE.md`](CODEBASE.md). Its graph maps every
  tracked input to directories, Cargo packages, Rust modules, dependencies, and curated
  responsibilities without including local build output or untracked developer material.
- Update the derived graph with `python3 scripts/generate-codebase-graph.py` after changing file
  layout, Cargo dependency declarations, file-backed Rust modules, or the curated architecture map.
- Treat the graph as a navigation aid. Source, task contract, and behavior checks remain
  authoritative.
