...
pub(crate) const MAX_SUBAGENTS: usize = 32;
const DEFAULT_AUTH_REFRESH_INTERVAL_MS: u64 = 300_000;
const DEFAULT_AUTH_COMMAND_TIMEOUT_MS: u64 = 310_000;
...
fn validate_max_subagents(limit: usize) -> Result<()> {
    if !(1..=MAX_SUBAGENTS).contains(&limit) {
        return Err(ConfigError::MaxSubagents(limit).into());
    }
    Ok(())
}
...
impl Config {
    ...
    fn load_with_options(
        ...
        let auth_command =
            Self::configured_path(None, file.auth.command.clone(), &path, current_dir);
        if auth_command.is_some() && file.auth.timeout_ms == Some(0) {
            return Err(ConfigError::AuthCommandTimeout.into());
        }
        ...
        let max_subagents = overrides
            .max_subagents
            .or(file.agent.max_subagents)
            .unwrap_or(DEFAULT_MAX_SUBAGENTS);
        validate_max_subagents(max_subagents)?;
        ...
        Ok(Self {
            ...
            .with_command(
                auth_command,
                file.auth.refresh_interval_ms,
                file.auth.timeout_ms,
            ),
            models,
            agent: AgentConfig {
                workspace,
    ...
    pub(crate) fn set_max_subagents(&mut self, limit: usize) -> Result<()> {
        validate_max_subagents(limit)?;
...
impl AuthConfig {
    ...
    fn with_command(
        ...
        if self.command.is_some() {
            self.refresh_interval_ms =
                Some(refresh_interval_ms.unwrap_or(DEFAULT_AUTH_REFRESH_INTERVAL_MS));
            self.timeout_ms = Some(timeout_ms.unwrap_or(DEFAULT_AUTH_COMMAND_TIMEOUT_MS));
        }
        self
    }
    ...
    pub(crate) fn command(&self) -> Option<&Path> {
    ...
    pub(crate) fn refresh_interval(&self) -> Duration {
        Duration::from_millis(
            self.refresh_interval_ms
                .unwrap_or(DEFAULT_AUTH_REFRESH_INTERVAL_MS),
        )
    }

    pub(crate) fn command_timeout(&self) -> Duration {
        Duration::from_millis(self.timeout_ms.unwrap_or(DEFAULT_AUTH_COMMAND_TIMEOUT_MS))
    }
}
...
impl ConfigFile {
    ...
    fn validate_secret_permissions(&mut self, path: &Path) -> Result<()> {
        ...
        if !self.contains_inline_secrets() {
            return Ok(());
        }
        let metadata = match fs::metadata(path) {
            Ok(metadata) => metadata,
            Err(source) => {
                self.zeroize_inline_secrets();
                return Err(ConfigError::Read {
                    path: path.to_path_buf(),
                    source,
                }
...
mod tests {
    ...
    fn missing_default_file_materializes_all_defaults() {
        ...
        for mode in ["light", "dark"] {
            assert_eq!(
                rendered["theme"][mode]["overlay_shadow"].as_str(),
                Some("#D6B1AB")
            );
        }
    ...
    #[test]
    fn astra_configuration_and_model_routes_round_trip() {
        for name in ["astra", "gpt-6-astra"] {
            let config = load_config(&format!(
                "[agent]\nmodel = \"{name}\"\nthinking = \"max\"\n\n[models.{name}]\napi_base_url = \"https://astra.example/v1\"\n"
            )).unwrap();
            assert_eq!(config.agent().model(), Model::Astra);
            assert_eq!(
                config.model_routes()[&Model::Astra].api_base_url.as_deref(),
                Some("https://astra.example/v1")
            );
            let rendered = config.to_toml().unwrap();
            let reloaded = load_config(&rendered).unwrap();
            assert_eq!(reloaded.agent().model(), Model::Astra);
            assert!(reloaded.model_routes().contains_key(&Model::Astra));
        }
    }
    ...
    #[test]
    fn auth_command_resolves_relative_to_config_and_loads_refresh_settings() {
        let directory = tempdir().unwrap();
        let config_path = directory.path().join("config.toml");
        fs::write(
            &config_path,
...
"#,
        ...
        assert_eq!(
            config.auth().command(),
            Some(directory.path().join("bin/token-helper").as_path())
        );
        assert_eq!(
            config.auth().refresh_interval(),
            Duration::from_millis(1234)
        );
        assert_eq!(config.auth().command_timeout(), Duration::from_millis(5678));
        let rendered = config.to_toml().unwrap();
        assert!(rendered.contains(r#"command = ""#));
        assert!(rendered.contains("refresh_interval_ms = 1234"));
        assert!(rendered.contains("timeout_ms = 5678"));
    }

    #[test]
    fn auth_command_rejects_zero_timeout() {
...
"#,
    ...
    #[test]
    fn invalid_max_subagents_reload_preserves_the_active_config() {
        let directory = tempdir().unwrap();
        ...
        assert!(matches!(
            config.reload(),
            Err(Error::Config(ConfigError::MaxSubagents(0)))
        ));
        assert_eq!(config.agent().max_subagents(), 8);
    }
    ...
    fn theme_overrides_are_loaded_and_serialized() {
        ...
        assert_eq!(config.theme.accent(), Color::Rgb(0x82, 0xAE, 0xF5));
...
"#,
    ...
    #[test]
    fn max_subagents_rejects_values_outside_the_runtime_range() {
        for limit in [0, 33] {
            let error = load_config(&format!("[agent]\nmax_subagents = {limit}\n")).unwrap_err();
            assert!(matches!(
                error,
                Error::Config(ConfigError::MaxSubagents(value)) if value == limit
            ));
        }
        ...
        assert!(matches!(
            Config::persist_max_subagents_at(&path, 33),
            Err(Error::Config(ConfigError::MaxSubagents(33)))
        ));
