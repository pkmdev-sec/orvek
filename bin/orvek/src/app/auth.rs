...
impl AuthConfig {
    ...
    pub(crate) async fn status(&self) -> AuthResult<()> {
        ...
        match self.select_auth(|| SecretString::from_environment(api_key_env))? {
            ...
            SelectedAuth::Command {
                ...
                .into_provider_auth(self.file())?
                .validate()
                .await?;
                println!("Authentication: API key command {display}");
            }
        }
        ...
        F: FnOnce() -> StdResult<Option<SecretString>, SecretError>,
    {
        match self.mode() {
            ...
            AuthMode::ApiKey => self.command_auth().map_or_else(
                || {
                    read_api_key()?
            ...
            AuthMode::Auto => {
                ...
                if let Some(command) = self.command_auth() {
                    return Ok(command);
                }
    ...
    fn command_auth(&self) -> Option<SelectedAuth> {
        self.command().map(|program| SelectedAuth::Command {
    ...
    fn into_provider_auth(self, auth_file: &Path) -> AuthResult<Auth> {
        match self {
            ...
            Self::Command {
                ...
                refresh_interval,
                timeout,
            } => Auth::api_key_command(program, refresh_interval, timeout).map_err(Into::into),
        }
    }
}
