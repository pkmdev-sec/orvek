...
impl QueueItemState {
    const fn label(self) -> &'static str {
        match self {
            ...
            Self::Updating => "syncing",
        }
    }

    fn color(self, theme: &Theme) -> Color {
        match self {
            Self::Queued => theme.success(),
            Self::Editing => theme.warning(),
            Self::Updating => theme.accent(),
        }
    }
}
...
impl Default for MessageQueue {
    fn default() -> Self {
        Self {
            ...
            area: Rect::default(),
            focused: false,
            updating_label: WavedText::new(UPDATING_TEXT, Color::Reset),
        }
    }
}
impl MessageQueue {
...
impl Component for MessageQueue {
    ...
    fn render(&mut self, frame: &mut Frame<'_>, area: Rect, theme: &Theme) {
        ...
        let title = if self.updating_label.is_active() {
            ...
            spans.extend(self.updating_label.spans_with_color(theme.accent()));
            spans.push(Span::styled(" ", Style::default().fg(border)));
            Line::from(spans)
        } else {
            Line::styled(
                format!(" queue {} · enter steer latest ", self.items.len()),
        ...
        let show_state = area.width >= 34;
        let prompt_width = usize::from(area.width).saturating_sub(if show_state { 16 } else { 7 });
        let rows = self
            .items
            .iter()
            .enumerate()
            ...
            .map(|(index, item)| {
                ...
                } else if item.state != QueueItemState::Queued {
                    Style::default()
                        .fg(theme.muted())
                ...
                let mut cells = vec![
                    Cell::from(marker),
                    Cell::from(truncate(item.prompt.display_text(), prompt_width).into_owned()),
                ];
                if show_state {
                    cells.push(
                        Cell::from(item.state.label())
                            .style(Style::default().fg(item.state.color(theme))),
                    );
                }
                Row::new(cells).style(row_style).height(2)
            })
            .collect::<Vec<_>>();
        let widths = if show_state {
            vec![
                Constraint::Length(1),
                Constraint::Min(8),
                Constraint::Length(8),
...
mod tests {
    ...
    #[test]
    fn focused_row_uses_an_accent_marker_without_reverse_video() {
        ...
        let mut terminal =
            ratatui::Terminal::new(ratatui::backend::TestBackend::new(48, 7)).unwrap();
        terminal
            .draw(|frame| queue.render(frame, frame.area(), &theme))
            .unwrap();

        let row = &terminal.backend().buffer().content[3 * 48..4 * 48];
        assert!(row.iter().any(|cell| cell.symbol() == "▸"));
        let selected = row.iter().find(|cell| cell.symbol() == "s").unwrap();
    ...
    fn wheel_reaches_hidden_queue_items_and_clicks_keep_their_identity() {
        ...
        let initial = rendered_queue(&mut queue, 7);
        assert!(initial.contains("queued item 0"));
        assert!(initial.contains("ready"), "{initial}");
        for _ in 0..20 {
