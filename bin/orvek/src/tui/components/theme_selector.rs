...
impl ThemeSelector {
    pub(super) fn new(initial: ThemeMode) -> Self {
        ...
        let mut choice = ChoicePicker::new(ThemeMode::ALL.len(), 1);
        choice.move_by(isize::try_from(selected).unwrap_or(isize::MAX));
        Self { choice }
    }
    ...
    fn update_key(
        ...
        match key.code {
            ...
            KeyCode::Up | KeyCode::Left => self.select_bounded(-1),
            KeyCode::Down | KeyCode::Right => self.select_bounded(1),
            KeyCode::Enter => ComponentUpdate {
                effects: vec![ThemeSelectorEffect::Apply(
                    ThemeMode::ALL[self.choice.selected_or_zero()],
                )],
...
impl Component for ThemeSelector {
    ...
    fn render(&mut self, frame: &mut Frame<'_>, area: Rect, theme: &Theme) {
        ...
        let layout = Dialog::new("Theme", 38, 7, &KEY_BINDINGS).render(frame, area, theme);
        self.choice.set_area(layout.body);
        ...
        self.choice
            .render(frame, layout.body, items.collect(), true, theme);
        let cursor_y =
            layout.body.y + u16::try_from(self.choice.selected_or_zero()).unwrap_or(u16::MAX);
