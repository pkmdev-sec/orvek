...
impl Component for KeybindingsHelp {
    ...
    fn render(&mut self, frame: &mut Frame<'_>, area: Rect, theme: &Theme) {
        let height = u16::try_from(BINDINGS.len())
            ...
            .saturating_add(3);
        let layout =
            Dialog::new("Keyboard shortcuts", 88, height, &FOOTER).render(frame, area, theme);
        self.body = layout.body;
        if self.body.width >= 56 {
            self.render_table(frame, theme);
        } else {
            self.render_wrapped(frame, theme);
        }
    }
}

impl KeybindingsHelp {
    fn render_table(&mut self, frame: &mut Frame<'_>, theme: &Theme) {
        self.max_scroll = BINDINGS
            .len()
            .saturating_sub(usize::from(self.body.height))
            .min(usize::from(u16::MAX)) as u16;
        self.scroll = self.scroll.min(self.max_scroll);
        let rows = BINDINGS.iter().map(|&(key, description)| {
            Row::new([
                Cell::from(Line::styled(format!(" {key}"), typography::key(theme))),
                Cell::from(Line::styled(description, typography::secondary(theme)).right_aligned()),
            ])
        });
        let mut state = TableState::default().with_offset(usize::from(self.scroll));
        let table_area = if self.max_scroll > 0 && self.body.width > 1 {
            Rect {
                width: self.body.width - 1,
                ..self.body
            }
        } else {
            self.body
        ...
        frame.render_stateful_widget(
            Table::new(rows, [Constraint::Length(22), Constraint::Min(1)]).column_spacing(1),
            table_area,
            &mut state,
        );
        self.render_scrollbar(frame, BINDINGS.len(), usize::from(self.body.height), theme);
    }

    fn render_wrapped(&mut self, frame: &mut Frame<'_>, theme: &Theme) {
        let lines = BINDINGS
            .iter()
            .map(|&(key, description)| binding_line(key, description, self.body.width, theme))
            .collect::<Vec<_>>();
        let paragraph = Paragraph::new(lines).wrap(Wrap { trim: false });
        let line_count = paragraph.line_count(self.body.width);
        self.max_scroll = line_count
            .saturating_sub(usize::from(self.body.height))
            .min(usize::from(u16::MAX)) as u16;
        self.scroll = self.scroll.min(self.max_scroll);
        frame.render_widget(paragraph.scroll((self.scroll, 0)), self.body);
        self.render_scrollbar(frame, line_count, usize::from(self.body.height), theme);
    }
    ...
    fn render_scrollbar(
        ...
        if content_length <= viewport_length || self.body.is_empty() {
            return;
        }
        ...
        frame.render_stateful_widget(scrollbar, self.body, &mut state);
    }
}
...
mod tests {
    ...
    fn popup_right_aligns_muted_descriptions() {
        ...
        let line = &rendered[row];
        let key_x = u16::try_from(unicode_width::UnicodeWidthStr::width(
            &line[..line.find("ctrl+s").unwrap()],
        ))
        .unwrap();
        let description_end =
            line.find("change reasoning effort").unwrap() + "change reasoning effort".len();
        let description_x =
            u16::try_from(unicode_width::UnicodeWidthStr::width(&line[..description_end]) - 1)
                .unwrap();
        let row = u16::try_from(row).unwrap();
        assert_eq!(buffer[(key_x, row)].fg, Theme::default().accent());
        assert_eq!(buffer[(description_x, row)].fg, Theme::default().muted());
    }
