...
use super::markdown::{Layout, SourceSpan, plain_selection_spans_excluding, sanitize, wrap_plain};
use crate::tui::{
...
pub(super) fn summary_position(tool: &ToolEntry, width: u16) -> Position {
    if tool.name == "exec_command" && width >= 8 {
        Position::new(2, 1)
    } else {
        Position::default()
    }
...
fn summary_lines(
    ...
    let position = summary_position(tool, width);
    let inner_width = width.saturating_sub(position.x * 2);
    let mut lines = unframed_summary_lines(
        tool,
        presentation,
        live_duration_ns,
        inner_width,
    ...
    for line in &mut lines {
        let padding = usize::from(inner_width).saturating_sub(line.width());
    ...
    lines.insert(0, Line::from(Span::styled(format!("╭{rule}╮"), border)));
    lines.push(Line::from(Span::styled(format!("╰{rule}╯"), border)));
    lines
}

fn unframed_summary_lines(
    tool: &ToolEntry,
    presentation: &Presentation,
    live_duration_ns: Option<u64>,
    width: u16,
    ...
    let no_arguments =
        matches!(&presentation.subject, Subject::Plain(subject) if subject == "no arguments");
    let mut leading = vec![Span::styled(if expanded { "▼ " } else { "▶ " }, border)];
    append_span(
        &mut leading,
        &presentation.title,
        Style::default()
    ...
    if !no_arguments {
        push_subject(&mut leading, &presentation.subject, theme);
    }
    let preserve_outcome = matches!(presentation.summary_overflow, SummaryOverflow::Truncate);
    if !preserve_outcome && let Some(outcome) = &presentation.outcome {
        append_span(
            &mut leading,
            &format!(" · {outcome}"),
    ...
    let error = (tool.state == ToolState::Failed)
        .then(|| first_error_line(tool.result.as_ref()))
        .flatten();
    ...
    suffix.push(Span::styled(status_symbol(tool.state), status));
    ...
    let minimum = suffix_width.saturating_add(1);
    let mut line = if width <= minimum {
        truncate_spans_with_ellipsis(
            &leading.into_iter().chain(suffix).collect::<Vec<_>>(),
            width,
            Style::default().fg(theme.muted()),
        )
    } else {
        let leading_width = width.saturating_sub(minimum);
        let mut rendered = if spans_need_truncation(&leading, leading_width) {
            truncate_spans_with_ellipsis(
                &leading,
                leading_width,
                Style::default().fg(theme.muted()),
        ...
        let used = spans_width(&rendered);
        rendered.push(Span::raw(" ".repeat(usize::from(
            width.saturating_sub(used).saturating_sub(suffix_width),
        ))));
        ...
        && width >= 3
    {
        ...
        lines.extend(details.into_iter().map(|line| {
            Line::from(
    ...
    if expanded && no_arguments && width >= 3 {
        lines.push(Line::from(vec![
            Span::styled("│  ", border),
            Span::styled("no arguments", Style::default().fg(theme.muted())),
        ]));
    }
    lines
}

pub(super) fn live_status_column(
    tool: &ToolEntry,
    duration_ns: u64,
    width: u16,
    theme: &Theme,
    ...
    .into_iter()
    .nth(usize::from(summary_position(tool, width).y))?;
    let symbol = status_symbol(tool.state);
    let mut column = 0_u16;
    for span in line.spans {
        if span.content == symbol {
            return Some(column);
        }
...
mod tests {
    ...
    #[test]
    fn argument_free_tool_stays_on_one_compact_row() {
        let lines = render(&tool("task_status", json!({})), 60, &Theme::default());

        assert_eq!(lines.len(), 1);
        assert!(lines[0].to_string().starts_with("▶ Task status"));
        assert!(lines[0].to_string().ends_with("✓  1.2s"));
        let expanded = render_expanded(&tool("task_status", json!({})), 60, &Theme::default());
        assert!(
            expanded
                .iter()
                .any(|line| line.to_string().contains("no arguments"))
        );
    }
    ...
    #[test]
    fn completed_shell_has_a_boxed_command_summary() {
        let mut shell = tool(
            "exec_command",
        ...
        assert!(summary.starts_with("│ ▶ Shell  $ cargo test"));
        assert!(summary.ends_with("exit 0  ✓  1.2s │"));
        let checkmark = lines[1]
            .spans
            .iter()
            .find(|span| span.content == "✓")
            .expect("successful tool should render a checkmark");
        assert_eq!(checkmark.style.fg, Some(Theme::default().success()));
    }

    #[test]
    fn shell_frames_keep_live_summary_metadata_and_narrow_widths_consistent() {
        let mut shell = tool(
            "exec_command",
            json!({"cmd": "if test \"$HOME\"; then echo ok; fi"}),
        );
        shell.state = ToolState::Running;
        for width in [0, 1, 2, 7, 8, 20, 80, 200] {
            for expanded in [false, true] {
                let theme = Theme::default();
                let layout = render_layout(&shell, Some(3_000_000_000), width, &theme, expanded);
                let summary =
                    super::render_live_summary(&shell, 3_000_000_000, width, &theme, expanded);
                assert_eq!(&layout.lines[..summary.len()], &summary);
                ...
                assert!(layout.selections[..summary.len()].iter().all(Vec::is_empty));
                if width >= 8 {
                    assert!(summary[0].to_string().starts_with('╭'));
                    assert!(summary[1].to_string().starts_with("│ ▶") || expanded);
                    assert!(summary[2].to_string().starts_with('╰'));
                } else {
                    assert!(summary.iter().all(|line| !line.to_string().contains('╭')));
                }
            }
        }
    }
    ...
    fn code_workflow_uses_the_compact_workflow_label() {
        ...
        assert!(lines[0].to_string().starts_with("▶ Batch  2 tools"));
        assert!(lines[0].to_string().ends_with("✓  1.2s"));
        assert!(lines.iter().all(|line| line.width() <= 80));
    }

    #[test]
    fn non_shell_tool_summary_compacts_overflow_without_losing_status() {
        let operation = tool(
            "custom_operation",
        ...
        assert!(rendered.ends_with("✓  1.2s"));
    }
    ...
    fn shell_summaries_truncate_subject_and_preserve_status_suffix() {
        ...
        let shell_line = render(&shell, 36, &Theme::default()).remove(1);
        let live_shell_line =
            render_live(&shell, 2_500_000_000, 36, &Theme::default(), false).remove(1);
    ...
    fn shell_summary_truncates_at_the_first_explicit_newline() {
        ...
        assert!(
            lines[1]
                .to_string()
                .starts_with("│ ▶ Shell  $ printf one …")
        );
        assert!(lines[1].to_string().ends_with("✓  1.2s │"));
    }
    ...
    fn collapsed_failure_includes_the_first_error_line() {
        ...
        assert!(lines[2].to_string().contains("compilation failed"));
        assert!(
            lines
                .iter()
                .all(|line| !line.to_string().contains("more diagnostics"))
        );
    }
    ...
    fn killed_shell_renders_failure_without_a_checkmark() {
        ...
        let rendered = render(&shell, 80, &Theme::default())[1].to_string();
