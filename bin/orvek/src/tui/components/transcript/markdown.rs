...
mod tests {
    ...
    fn requested_markdown_styles_are_applied() {
        ...
        assert_eq!(lines[0].spans[0].style.fg, Some(theme.thinking_max()));
        let code = lines[2]
            .spans
            .iter()
            .find(|span| span.content.contains("code"))
            .unwrap();
        assert_eq!(code.style.fg, Some(theme.code_text()));
    ...
    fn unsupported_image_backends_render_images_as_markdown_links() {
        ...
        let label = layout.lines[0]
            ...
            .unwrap();
        assert_eq!(label.style.fg, Some(Theme::default().accent()));
