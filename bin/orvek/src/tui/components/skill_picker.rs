...
impl SkillPicker {
    pub(super) fn new(skills: Arc<[Skill]>) -> Self {
        let matches = (0..skills.len()).collect::<Vec<_>>();
        Self {
            skills,
            query: String::new(),
    ...
    fn render_skills(&mut self, frame: &mut Frame<'_>, area: Rect, theme: &Theme) {
        if area.is_empty() {
            return;
        }
...
impl Component for SkillPicker {
    ...
    fn render(&mut self, frame: &mut Frame<'_>, area: Rect, theme: &Theme) {
        ...
        let layout = Dialog::new("Skills", 72, 14, &KEY_BINDINGS).render(frame, area, theme);
        ...
        self.choice.set_area(skills_area);
