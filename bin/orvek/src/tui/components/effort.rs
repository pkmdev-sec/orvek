...
impl EffortSelector {
    ...
    fn render_labels(&self, frame: &mut Frame<'_>, area: Rect, theme: &Theme) {
        ...
        let lines = vec![
            ...
            Line::from(vec![
                Span::styled("Pro: ", Style::default().fg(theme.success())),
                Span::styled(
...
impl Component for EffortSelector {
    ...
    fn render(&mut self, frame: &mut Frame<'_>, area: Rect, theme: &Theme) {
        ...
        let layout = Dialog::new("Effort", 48, 17, &KEY_BINDINGS).render(frame, area, theme);
...
mod tests {
    ...
    fn dial_has_five_evenly_spaced_stops_with_one_at_the_top() {
        ...
        assert_eq!(
            buffer[(29, 2)].fg,
            Theme::default().effort(ReasoningEffort::Low)
        );
        assert_eq!(buffer[(20, 12)].fg, Theme::default().border());
        let footer = (6..54)
    ...
    fn animation_fills_thick_dots_in_the_new_effort_color() {
        ...
        terminal
            ...
            .unwrap();
        assert_eq!(
            terminal.backend().buffer()[(29, 2)].fg,
            Theme::default().effort(ReasoningEffort::Medium)
        );
        assert_eq!(terminal.backend().buffer()[(37, 7)].symbol(), "•");
        assert_eq!(
            terminal.backend().buffer()[(37, 7)].fg,
            Theme::default().muted()
        );
        ...
        assert_eq!(
            terminal.backend().buffer()[(29, 2)].fg,
            Theme::default().effort(ReasoningEffort::High)
        );
        assert_eq!(
            terminal.backend().buffer()[(37, 7)].fg,
            Theme::default().muted()
    ...
    fn max_to_low_progressively_removes_the_colored_arc() {
        ...
        for elapsed in [Duration::ZERO, ANIMATION_DURATION / 2, ANIMATION_DURATION] {
            ...
            colored.push(colored_dial_dots(
                &terminal,
                Theme::default().effort(ReasoningEffort::Low),
            ));
        }
    ...
    fn max_to_low_drains_clockwise_from_low_toward_max() {
        ...
        let buffer = terminal.backend().buffer();
        assert_eq!(buffer[(37, 5)].fg, Theme::default().muted());
        assert_eq!(
            buffer[(21, 5)].fg,
            Theme::default().effort(ReasoningEffort::Low)
        );
    ...
    fn low_to_max_is_the_reverse_of_the_forward_wrap() {
        ...
        for elapsed in [Duration::ZERO, ANIMATION_DURATION / 2, ANIMATION_DURATION] {
            ...
            colored.push(colored_dial_dots(
                &terminal,
                Theme::default().effort(ReasoningEffort::Max),
            ));
        }
    ...
    fn pro_state_and_toggle_help_are_green() {
        ...
        let pro_start = row
            ...
            .unwrap();
        assert!((pro_start..pro_start + 3).all(|x| row[x].fg == Theme::default().success()));
