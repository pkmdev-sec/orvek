...
impl ActionsMenu {
    pub(super) fn new(availability: ActionAvailability) -> Self {
        Self {
            query: String::new(),
            choice: ChoicePicker::new(ACTIONS.len(), 1),
    ...
    fn trigger_selected(&self) -> ComponentUpdate<ActionsEffect> {
        let Some(action) = self.matches.get(self.choice.selected_or_zero()) else {
            return ComponentUpdate::none();
        };
    ...
    fn render_actions(&mut self, frame: &mut Frame<'_>, area: Rect, theme: &Theme) {
        if area.is_empty() {
            return;
        }
        ...
        let selected_enabled = self
            .matches
            .get(self.choice.selected_or_zero())
            .is_some_and(|index| self.is_enabled(ACTIONS[*index]));
        self.choice
            .render(frame, area, items.collect(), selected_enabled, theme);
    }
...
impl Component for ActionsMenu {
    ...
    fn render(&mut self, frame: &mut Frame<'_>, area: Rect, theme: &Theme) {
        ...
        let layout = Dialog::new("Actions", 58, 19, &KEY_BINDINGS).render(frame, area, theme);
        ...
        self.choice.set_area(actions_area);
...
mod tests {
    ...
    fn arrows_navigate_while_typing_continues_to_search() {
        ...
        assert_eq!(menu.choice.selected_or_zero(), 1);
        assert_eq!(
        ...
        assert_eq!(menu.choice.selected_or_zero(), 0);
    }
