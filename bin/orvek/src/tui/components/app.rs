...
impl AppNode {
    ...
    pub(crate) fn render(&mut self, frame: &mut Frame<'_>) {
        ...
        if area.width < 65 {
            [self.main_area, self.fork_area] =
                Layout::vertical([Constraint::Fill(1), Constraint::Fill(1)])
                    .spacing(1)
                    .flex(Flex::Center)
                    .areas(area);
            let divider_y = self.main_area.bottom();
            if let Some((main_pane, main)) = &mut self.main {
                main.component_mut().render_focused(
                    frame,
                    self.main_area,
        ...
        [self.main_area, self.fork_area] =
            Layout::horizontal([Constraint::Fill(1), Constraint::Fill(1)])
                ...
                .flex(Flex::Center)
                .areas(area);
        let divider_x = self.main_area.right();
...
mod tests {
    ...
    fn wheel_on_the_split_divider_or_outside_the_frame_does_nothing() {
        let mut app = scrollable_split_app();
        let before = rendered_panes(&mut app);
        let divider = app.main_area.right();
        for (column, row) in [(divider, 6), (100, 6), (10, 24)] {
    ...
    fn auto_theme_redraws_for_system_changes_but_explicit_modes_ignore_them() {
        ...
        assert_eq!(
            ...
            ratatui::style::Color::Rgb(0xE3, 0xD4, 0xC1)
        );
        ...
        assert_eq!(
            app.theme.code_background(),
            ratatui::style::Color::Rgb(0x21, 0x1D, 0x2A)
        );
    }
    ...
    fn split_view_shows_mouse_focus_and_close_hint() {
        ...
        assert_eq!(app.main_area.y, app.fork_area.y);
        assert!(app.main_area.x < app.fork_area.x);
    }
    ...
    fn split_hint_is_hidden_at_narrow_widths() {
        ...
        assert!(!rendered.contains(SPLIT_HINT.trim()));
        assert_eq!(app.main_area.x, app.fork_area.x);
        assert!(app.main_area.y < app.fork_area.y);
    }
