...
impl ModelSelector {
    ...
    pub(super) fn animation_deadline(&self) -> Option<Instant> {
        self.animation
            .as_ref()
            .and_then(|animation| animation.timeline.deadline())
    }
    ...
    fn select_relative(
        ...
        self.selected = next;
        let mut timeline = AnimationTimeline::new(ANIMATION_DURATION, ANIMATION_FRAME_INTERVAL);
        timeline.start(now);
    ...
    fn advance_animation(&mut self, now: Instant) -> bool {
        ...
        let sample = animation
            .timeline
            .advance(now)
            .expect("an active animation has a running timeline");
        let eased = sample.ease_out_cubic();
    ...
    fn render_slider(&self, frame: &mut Frame<'_>, area: Rect, theme: &Theme) {
        ...
        let labels = [
            ...
            (model_column(left, width, 5), Model::Astra, "Astra"),
        ];
        for (column, model, label) in labels {
            if width < 6 * (MODELS.len() - 1) as u16 && model != MODELS[self.selected] {
                continue;
            }
...
impl Component for ModelSelector {
    ...
    fn render(&mut self, frame: &mut Frame<'_>, area: Rect, theme: &Theme) {
        ...
        let layout = Dialog::new("Select model", 52, 7, &KEY_BINDINGS).render(frame, area, theme);
...
mod tests {
    ...
    fn every_stop_keeps_its_model_color() {
        ...
        assert_eq!(
            rendered_label_color(&mut selector, "Luna"),
            theme.model(Model::Luna)
        );
        assert_eq!(
            rendered_label_color(&mut selector, "Terra"),
            theme.model(Model::Terra)
        );
        assert_eq!(
            rendered_label_color(&mut selector, "Sol"),
            theme.model(Model::Sol)
        );
        assert_eq!(
            rendered_label_color(&mut selector, "GLM"),
            theme.model(Model::Glm)
        );
        assert_eq!(
            rendered_label_color(&mut selector, "Spark"),
            theme.model(Model::Spark)
        );
        assert_eq!(
            rendered_label_color(&mut selector, "Astra"),
            theme.model(Model::Astra)
        );
    }
    ...
    fn filled_bar_uses_the_selected_model_color() {
        ...
        assert!(
            rail.iter()
                .all(|cell| cell.fg == Theme::default().model(Model::Astra))
        );
    }
    ...
    fn stops_use_the_filled_bar_color_only_when_covered() {
        let theme = Theme::default();
        for (selected, model) in MODELS.into_iter().enumerate() {
            let expected: Vec<_> = (0..MODELS.len())
                ...
                .collect();
            assert_eq!(
                rendered_stop_colors(&mut ModelSelector::new(model)),
                expected
            );
        }
    }
    ...
    #[test]
    fn astra_is_visible_and_selectable() {
        let now = Instant::now();
        ...
        assert_eq!(
            ...
            [ModelSelectorEffect::Apply(Model::Astra)]
        );
        let terminal = render(&mut selector);
        let text: String = terminal
            ...
            .map(|cell| cell.symbol())
            .collect();
        assert!(text.contains("Selected: Astra"));
        assert_eq!(
            rendered_label_color(&mut selector, "Astra"),
            Theme::default().model(Model::Astra)
        );
    }

    #[test]
    fn astra_label_fits_a_narrow_selector() {
        let mut terminal = Terminal::new(TestBackend::new(30, 9)).unwrap();
        terminal
            .draw(|frame| {
                ModelSelector::new(Model::Astra).render(frame, frame.area(), &Theme::default());
            })
            .unwrap();
        let buffer = terminal.backend().buffer();
        let text: String = buffer.content.iter().map(|cell| cell.symbol()).collect();
        assert!(text.contains("Selected: Astra"));
        assert_eq!(text.matches("Astra").count(), 2);
        assert!(!text.contains("SparkAstra"));
    }
