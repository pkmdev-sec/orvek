...
impl TranscriptModel {
    ...
    fn apply_host(&mut self, change: &ViewChange, at_ms: u64) -> bool {
        match change {
            ...
            ViewChange::Assistant {
                ...
                let id = match self.assistants.get(&key).copied() {
                    ...
                    None => {
                        let preview = confirmed
                            .then(|| self.sole_pending_assistant(*request))
                            .flatten();
                        let id = match preview {
                            Some((preview_key, id)) => {
                                self.assistants.remove(&preview_key);
                                id
                            }
                            None => self.push(EntryKind::Assistant {
                                text: String::new(),
                                complete: false,
                                final_answer: false,
                            }),
                ...
                self.update(id, |kind| {
                    ...
                    } = kind
                    {
                        ...
                        *complete = *confirmed;
                        *current_final_answer = *confirmed && *final_answer;
                    }
                });
            ...
            ViewChange::TaskLinked(task) => {
                self.push(EntryKind::TaskStatus {
                    id: *task,
            ...
            ViewChange::Task { id, event } => {
                ...
                let (text, verified) = match event {
                    ...
                    | TaskEvent::ContractAmended { contract, .. } => (
                        format!(
                            "{} required outcomes · {} acceptance checks",
                            contract.requirements.len(),
                            contract.checks.len()
                        ),
                        false,
                    ),
                    TaskEvent::PhaseChanged(phase) => (format!("{phase:?}"), false),
                    TaskEvent::Completed(certificate) => (
                        format!(
                            "complete · artifact {} · {} accepted evidence records",
                    ...
                    TaskEvent::Stopped { outcome, reason } => {
                        (format!("{} · {reason}", outcome_name(*outcome)), false)
                    }
                    TaskEvent::EvidenceInvalidated { reason }
                    | TaskEvent::WorkspaceChanged { reason } => {
                        (format!("evidence stale · {reason}"), false)
                    }
            ...
            ViewChange::ContextProjected { .. } => return false,
    ...
    fn sole_pending_assistant(&self, request: Option<Uuid>) -> Option<(MessageKey, EntryId)> {
        let request = request?;
        let mut pending = self.assistants.iter().filter(|((owner, _), id)| {
            *owner == Some(request)
        ...
        let (key, id) = pending.next()?;
        pending.next().is_none().then(|| (key.clone(), *id))
    }

    fn push(&mut self, kind: EntryKind) -> EntryId {
        if let Some(previous) = self.entries.last_mut()
            && !previous.trailing_spacer
            && (matches!(
                kind,
                ...
                previous.kind,
                EntryKind::User { .. } | EntryKind::Assistant { .. } | EntryKind::Reasoning { .. }
            ) && !matches!(kind, EntryKind::TurnCompleted { .. })))
        {
            previous.trailing_spacer = true;
            previous.revision = previous.revision.saturating_add(1);
        }
        let trailing_spacer = matches!(kind, EntryKind::TurnCompleted { .. });
...
mod tool_preview_tests {
    ...
    #[test]
    fn confirmed_phase_reconciles_preview_identity_and_final_answer_metadata() {
        for phase in ["final_answer", "commentary", "analysis"] {
            for confirmed_item in ["preview", "recorded"] {
                let mut model = TranscriptModel::default();
                let request = Uuid::new_v4();
                apply(
                    &mut model,
                ...
                let preview_id = model.entries()[0].id;
                let preview_revision = model.entries()[0].revision;
                assert!(matches!(
                ...
                let changes = history_items(
                    Some(request),
                    &[json!({
                        "type":"message", "id":confirmed_item, "role":"assistant",
                        "phase":phase, "content":"Recorded output"
                    })],
                );
                apply(&mut model, changes.clone());
                assert_eq!(model.entries().len(), 1);
                let entry = &model.entries()[0];
                assert_eq!(entry.id, preview_id);
                assert!(entry.revision > preview_revision);
                assert!(matches!(&entry.kind, EntryKind::Assistant {
                    text, complete: true, final_answer,
                } if text == "Recorded output" && *final_answer == (phase == "final_answer")));
                apply(&mut model, changes);
                assert_eq!(model.entries().len(), 1);
                assert!(
                    matches!(model.fork_snapshot().entries()[0].kind, EntryKind::Assistant {
                    final_answer, ..
                } if final_answer == (phase == "final_answer"))
                );
            }
        }
    }
    ...
    #[test]
    fn unconfirmed_assistant_is_never_a_final_answer() {
        let mut model = TranscriptModel::default();
        apply(
            &mut model,
            vec![ViewChange::Assistant {
                request: Some(Uuid::new_v4()),
                item: "preview".into(),
                text: "Draft".into(),
                replace: false,
                confirmed: false,
        ...
        apply(&mut model, vec![ViewChange::DiscardPreviews]);
        assert!(matches!(
            model.entries()[0].kind,
            EntryKind::Assistant {
                complete: false,
    ...
    #[test]
    fn ownerless_confirmed_answer_does_not_reconcile_an_unscoped_preview() {
        let mut model = TranscriptModel::default();
        apply(
            &mut model,
            vec![ViewChange::Assistant {
    ...
    #[test]
    fn context_projection_does_not_add_routine_transcript_noise() {
        let mut model = TranscriptModel::default();
        let change = apply(
            &mut model,
