                    ...
                    match effect {
                                ...
                                .await
                                {
                                    Ok(prepared) => {
                                        ...
                                        match prepare_install(prepared.configured).await {
                                            Ok(session) => {
                                                Ok(PreparedHandoffSession { prompt, session })
                                            }
                                            Err(error) => {
                                                Err(super::handoff_controller::HandoffFailure {
                                                    prompt: Some(prompt),
                                                    error: Box::new(error),
                                                })
                                            }
                        ...
                        RootEffect::ResumeSession(id) => {
                            ...
                            jobs.spawn(async move {
                                let result = async {
                                    let configured =
                                        ConfiguredSession::resume_label(&config, &id).await?;
                                    prepare_install(configured).await
                                }
                                .await;
                                let _ = out
            ...
            Some(update)=updates.recv()=>match update {
                Update::Handoff {pane,generation,result}=>{
                    if panes.get(&pane).is_some_and(|current|current.generation==generation) {
                        ...
                        match *result {
                            Ok(prepared)=>{
                                let view=prepared.session.configured.session.clone();let skills=prepared.session.configured.skills.clone();
                                ...
                                install(pane,replacement,prepared.session,&mut panes,&mut app,&sender);
                            }
                            Err(failure)=>{
...
fn watch(
    ...
    tokio::spawn(async move {
        ...
        loop {
            let result = tokio::select! {()=cancel.cancelled()=>return,result=client.subscribe(after, session)=>result};
...
fn is_image_paste_shortcut(event: &Event) -> bool {
