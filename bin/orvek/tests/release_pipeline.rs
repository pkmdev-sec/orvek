...
fn library_crates_are_published_before_orvek() {
    ...
    for expected in [
        ...
        "cargo package --locked -p orvek-subagents",
        "publish_package orvek-subagents",
    ...
    let subagents = publish.find("publish_package orvek-subagents").unwrap();
    let signed_manifest = publish.find("cp \"${RUNNER_TEMP}").unwrap();
    let orvek = publish.find("publish_package orvek --allow-dirty").unwrap();
    assert!(memory < signed_manifest && subagents < signed_manifest && signed_manifest < orvek);
}
